...
export function InviteButton({ inviteCode, groupName, eventName }: InviteButtonProps) {
  const [copied, setCopied] = useState(false)
  const inviteUrl = `${APP_URL}/join/${inviteCode}`
  const displayName = eventName ?? groupName
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: `Join "${displayName}" on Memoalive`,
          text:  `You've been invited to share memories in "${displayName}".`,
          url:   inviteUrl,
        })
        return
      } catch {
