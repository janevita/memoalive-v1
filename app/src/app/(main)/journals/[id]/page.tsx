...
export default async function JournalPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-canvas">
      {/* Header */}
      <div className="px-6 py-4 border-b-[2.5px] border-ink sticky top-0 z-40" style={{ background: '#1C1917' }}>
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <Link href={ROUTES.journals} className="text-white/50 hover:text-white transition-colors text-sm font-semibold flex-shrink-0">
              ← Journals
            </Link>
            <span className="text-white/20 flex-shrink-0">/</span>
            <div className="min-w-0">
              <p className="font-serif font-bold text-white text-sm truncate">{journal.title}</p>
              <p className="text-white/40 text-xs">About {journal.subjectName}</p>
            </div>
          </div>
