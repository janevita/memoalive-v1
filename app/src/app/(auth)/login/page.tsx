    ...
    <div className="min-h-screen bg-canvas flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <p className="mt-2 text-sm text-ink-soft">{inviteCode ? 'Sign in to join the group' : 'Welcome back'}</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-ink-soft">
          New to Memoalive?{' '}
          <Link href={inviteCode ? `${ROUTES.signup}?invite=${inviteCode}` : ROUTES.signup} className="text-flame font-semibold hover:underline">Create account</Link>
        </p>
      </div>
    </div>
  )
