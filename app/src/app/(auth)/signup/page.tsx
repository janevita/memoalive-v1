      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          <Link href={ROUTES.home} className="inline-block">
            <span className="font-serif text-4xl font-bold text-ink">Memo<span className="text-flame">alive</span></span>
          </Link>
          <p className="mt-2 text-sm text-ink-soft">{inviteCode ? 'Create a free account to join the group' : 'Start keeping memories with your people'}</p>
        </div>
        <form action={action} className="space-y-4">
          ...
          <div>
            ...
            <input name="name" type="text" className="input" placeholder="Rosa Costa" autoComplete="name" required />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="password" type="password" className="input" placeholder="Min. 8 characters" autoComplete="new-password" minLength={8} required />
          </div>
          <SubmitButton />
        </form>
        <p className="mt-6 text-center text-sm text-ink-soft">
          Already have an account?{' '}
          <Link href={inviteCode ? `${ROUTES.login}?invite=${inviteCode}` : ROUTES.login} className="text-flame font-semibold hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
